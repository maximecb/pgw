...
Drawing constraints come from site/style.css:
  ...
  - 820px content column, figure img at 95% of it, capped at 500px tall.
    On mobile the SVG is stretched to the full text column instead, so the
    canvas width below is a coordinate space, not the size it is seen at
...
# Blank space left at the left and right edges. The figures are laid out to
# put ink right up against it, since the browser scales the whole canvas to
# the text column and any slack inside here reads as the image being narrow.
MARGIN = 16

# DejaVu Sans Mono advance width, in em. Used to reserve room for text that
# hangs off the side of a block, so that centring the block does not leave
# the drawing lopsided. Every fallback in FONT is this wide or narrower, so
# a wrong guess costs a little whitespace rather than a clipped glyph.
MONO_ADV = 0.6022
...
def fig_pattern_grid():
    ...
    # The row labels are anchored at the grid's left edge and run leftwards,
    # and the bit count hangs off its right edge, so neither is inside the
    # grid. Sizing the cells around both is what lets the ink reach the
    # margins, rather than centring a block that is narrower than it looks.
    ann = f"{n} bits"
    labw = max(text_width(lab, 13) for lab, _ in rows)
    annw = text_width(ann, 12)
    gap, ann_gap = 12, 14
    cell = (W - 2 * MARGIN - labw - gap - ann_gap - annw) / n
    gw = n * cell
    lab_x = MARGIN + labw
    gx = lab_x + gap
    ...
    for ri, (lab, bits) in enumerate(rows):
        ry = ytop + ri * (cell + 3)
        b += text(lab_x, ry + cell / 2 + 5, lab, 13, INK, "end")
...
def fig_row_strategies():
    ...
    gw = n * cell
    gx = 244
    lx, rx = MARGIN, W - MARGIN
    panel_h = 72
    top = 56

    b = ""
    b += text(lx, 32, "SCHEME", 11.5, MID, ls="1")
    b += text(gx, 32, f"ROW ({n} STEPS)", 11.5, MID, ls="1")
    b += text(rx, 32, "COST", 11.5, MID, "end", ls="1")
    b += line(lx, 40, rx, 40, MID, STROKE)

    panels = panel_facts()
    for pi, (name, tag, bits, brk, cost, ghost) in enumerate(panels):
        py = top + pi * panel_h
        b += text(lx, py + 18, name, 12.5, INK, weight="bold")
        b += text(lx, py + 37, f"tag {tag}", 11.5, ON, weight="bold")
        b += text(lx, py + 53, brk, 10.5, SUB)
        b += step_grid(gx, py + 6, bits, cell, ghost=ghost)
        b += text(rx, py + 26, cost, 16, ON, "end", weight="bold")
        if pi < len(panels) - 1:
            b += line(lx, py + panel_h - 8, rx, py + panel_h - 8, LINE, HAIR)
    ...
    h = ry + panel_h + 16
    b += line(lx, h - 18, rx, h - 18, MID, STROKE)
    b += text(lx, h + 2,
...
def fig_timeline_rle():
    bars = 128
    # The ruler numbers are centred on their tick, so half of the first and
    # half of the last sit outside the strip. Inset the strip by that much,
    # and it is the numbers rather than the strip that reach the margin.
    gx = MARGIN + text_width(str(bars), 11) / 2
    gw = W - 2 * gx
    cell = gw / bars
    ...
    # Same span as the strip above, so the two line up down the figure
    bx, by, bw, bh = gx, y + 48, gw, 132
